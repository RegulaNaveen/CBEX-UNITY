...
describe('Answer History component', () => {
    beforeAll(() => {
        render(<Provider store={store}><AnswerHistory {...initalstate} /></Provider>)
      });
    test('Answer History component Load', async () => {
        const { container } = await render(<Provider store={store}><AnswerHistory {...initalstate} /></Provider>);
        expect(container.getElementsByClassName('modal-content').length).toBeGreaterThan(0);
    });
    ...
    test('Answer History Count', async () => {
        const { container } = await render(<Provider store={store}><AnswerHistory {...initalstate} /></Provider>);
        expect(container.getElementsByClassName('answer-container')).toHaveLength(5)
    });
    ...
    test('If Answer is empty', async () => {
        ...
        await render(<Provider store={dumystore}><AnswerHistory {...initalstate} /></Provider>);
    });
