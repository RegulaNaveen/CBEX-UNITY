...
describe('testing switch template component', () => {
  ...
  test(
    'change opportunity type with existing version',
    async () => {
      ...
      const selectedOpType = screen.getByRole('button', {
        name: 'Default Type'
      });
      ...
      waitFor(
        () => {
          const IQBTemplate = screen.getByText('Default Type');
  ...
  test('update opportunity type with latest version', async () => {
    ...
    expect(
      screen.getByText('Core Opportunity Launch Call (AMR/EMEA)')
    ).toBeInTheDocument();
