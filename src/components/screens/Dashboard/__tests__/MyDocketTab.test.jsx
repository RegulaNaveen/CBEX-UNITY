...
describe('testing my docket tab', () => {
  ...
  test('check for loaded proposals for current tab', () => {
    store.dispatch({
  ...
  test('check for loaded proposals for past tab with list view', () => {
  ...
  test('check for loaded proposals for past tab with card view', () => {
    store.dispatch({
      type: SET_PROPOSAL_VIEW_TYPE, payload: { typeView: 1 }
    });
    const { container } = render(
      <Provider store={store}>
        <SocketContext.Provider 
