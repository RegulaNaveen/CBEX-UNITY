...
import { CUSTOM_QUESTIONS_API_URL,CUSTOM_QUESTIONS_DELETE_API_URL } from '../constants/api';
...
export const setUnityQuestionData = async (
  ...
  export const deleteUnityQuestionData = async (
    ...
    return new Promise((resolve, reject) => {
      axiosInstance
        .post(`${CUSTOM_QUESTIONS_DELETE_API_URL}/${questionData.proposalId}`,questionData,{
          headers: { 'x-api-key': API_KEY, 'x-access-token': getAccessToken() }
        })
        .then(response => {
          resolve(response.data);
